...
export default function Books() {
  ...
  const bq = useQuery({ queryKey: ["books"], queryFn: booksApi.list });
  const cq = useQuery({ queryKey: ["copies"], queryFn: copiesApi.list });
  ...
  if (bq.isPending || cq.isPending) return <Loading />;
  if (bq.error)
    return <ErrorState error={bq.error} onRetry={() => bq.refetch()} />;
  return (
    <>
      <PageHeader
        title="Books & copies"
        ...
        action={
          <Button
            onClick={() =>
              tab === "books" ? setBookModal(null) : setCopyModal(null)
            }
          >
            <Plus size={17} /> Add {tab === "books" ? "book" : "copy"}
          </Button>
        }
      />
      <div className="tabs">
        ...
        >
          <BookOpen size={16} /> Titles <b>{books.length}</b>
        </button>
        <button
        ...
        >
          <Copy size={16} /> Physical copies <b>{copies.length}</b>
        </button>
      </div>
            ...
            <div className="book-card" key={book._id}>
              ...
              <div className="book-body">
                <div className="book-top">
                  <Badge tone={tone(book.status)}>{book.status}</Badge>
                  <span>{book.isbn}</span>
                </div>
                <h3>{book.title}</h3>
                ...
                <div className="card-actions">
                  <Button variant="ghost" onClick={() => setBookModal(book)}>
                    <Pencil size={15} /> Edit
                  </Button>
            ...
            <tbody>
              {(filtered as BookCopy[]).map((c) => (
                <tr key={c._id}>
                  ...
                  <td>
                    <Button variant="ghost" onClick={() => setCopyModal(c)}>
                      <Pencil size={15} />
                    </Button>
                  </td>
                </tr>
      ...
      {bookModal !== false && (
        <BookForm
          ...
          onSave={(body) => bm.mutate({ id: bookModal?._id, body })}
        />
      )}
      {copyModal !== false && (
        <CopyForm
          ...
          onSave={(body) => cm.mutate({ id: copyModal?._id, body })}
        />
      )}
    </>
  );
...
function BookForm({
  ...
  const [v, setV] = useState<any>(
    initial ? { ...initial, authors: initial.authors.join(",") } : emptyBook,
  );
  ...
  <div className="form-error full">
    {errorMessage(error)}
        ...
        <div className="form-actions full">
          ...
          <Button loading={busy}>Save book</Button>
        </div>
      </form>
    </Modal>
  );
...
function CopyForm({
  ...
  return (
    <Modal
      title={initial ? "Edit physical copy" : "Add physical copy"}
      onClose={onClose}
    >
      <form
        className="form-grid"
        onSubmit={(e) => {
          ...
          onSave({
            ...
            acquiredAt: v.acquiredAt || undefined,
          });
        }}
      >
        {!initial && (
        ...
        <Input
          label="Acquired at"
          type="date"
          value={v.acquiredAt?.slice?.(0, 10) || v.acquiredAt}
  ...
  <div className="form-error full">
    {errorMessage(error)}
        ...
        <div className="form-actions full">
          ...
          <Button loading={busy}>Save copy</Button>
        </div>
      </form>
    </Modal>
  );
